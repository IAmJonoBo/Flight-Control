...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-brand-light flex flex-col items-center justify-center">
      ...
      <h1 className="text-3xl font-bold text-brand-dark mb-2">Flight Control Dashboard</h1>
      <Settings selectedModel={selectedModel} onModelChange={setSelectedModel} />
      <div className="mb-6 w-full max-w-md">
        ...
        {gitResult && gitResult.error && (
          <div className="text-red-600 mt-2">{gitResult.error}</div>
        )}
      </div>
      <div className="w-full max-w-2xl mb-4">
        <CodeEditor value={code} onChange={setCode} />
        {analysisLoading && <div className="text-blue-600 mt-2">Analyzing...</div>}
        {analysisError && <div className="text-red-600 mt-2">{analysisError}</div>}
        {analysisResult && (
          <div className="mt-2 text-green-700">
            <pre className="bg-gray-100 p-2 rounded text-sm">{JSON.stringify(analysisResult, null, 2)}</pre>
          </div>
        )}
      </div>
    </div>
